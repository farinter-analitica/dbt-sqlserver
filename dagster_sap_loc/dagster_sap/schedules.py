...
dbt_dwh_sap_marts_job_schedule = ScheduleDefinition(
    ...
    cron_schedule = get_for_current_env(dict={"dev":"15 2 * * *","prd":"30 1 * * *"}),  #cron template: hour minute day month day_of_week
...
sap_etl_dwh_all_downstream_job_schedule = ScheduleDefinition(
    cron_schedule = get_for_current_env(dict={"dev":"15 01 * * *","prd":"05 00 * * *"}),  #cron template: hour minute day month day_of_week
    execution_timezone=default_timezone,
...
sap_etl_dwh_hourly_all_downstream_job_schedule = ScheduleDefinition(
    cron_schedule = get_for_current_env(dict={"dev":["05 6-18 * * *","15 23 * * *"],"prd":["05 6-18 * * *","15 23 * * *"]}),  #cron template: hour minute day month day_of_week
    execution_timezone=default_timezone,
