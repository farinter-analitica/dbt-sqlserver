...
def es_delta_max_datetime_superado(
    val_referencia: datetime | None,
    val_comprobado: datetime | None,
    delta_max: timedelta = timedelta(hours=1),
) -> bool:
...
def obtener_servidores_de_replica_en_alerta(
    ...
    # ldcom_cr_prd_sqlserver: SQLServerResource, #Esta replica ya no está activa
    ...
    lista_par_servidores: list[ParServidoresReplicaSQLServer] = [
        ...
        ParServidoresReplicaSQLServer(
            sql_server_origen=ldcom_hn_prd_sqlserver,
        ...
        ParServidoresReplicaSQLServer(
            sql_server_origen=ldcom_ni_prd_sqlserver,
            sql_server_replica=dwh_farinter_prd_replicas_ldcom,
        ...
        #                             ),
        ParServidoresReplicaSQLServer(
            sql_server_origen=ldcom_cr_arb_prd_sqlserver,
            sql_server_replica=dwh_farinter_prd_replicas_ldcom,
            relation_origen="dbo.Factura_Encabezado",
        ...
        ParServidoresReplicaSQLServer(
            sql_server_origen=ldcom_gt_prd_sqlserver,
            sql_server_replica=dwh_farinter_prd_replicas_ldcom,
            relation_origen="dbo.Ticket_Encabezado",
        ...
        ParServidoresReplicaSQLServer(
            sql_server_origen=ldcom_sv_prd_sqlserver,
            sql_server_replica=dwh_farinter_prd_replicas_ldcom,
            relation_origen="dbo.Ticket_Encabezado",
    ...
    for par_servidores in lista_par_servidores:
        ...
        if (
            es_delta_max_datetime_superado(max_val_origen, max_val_replica)
            or logs.get("ocurrio_error", None) is not None
        ):
            meta_data = {
                ...
                "max_val_origen": str(
                    max_val_origen.strftime("%Y-%m-%d %H:%M:%S")
                    if max_val_origen is not None
                    else max_val_origen
                ),
                "max_val_replica": str(
                    max_val_replica.strftime("%Y-%m-%d %H:%M:%S")
                    if max_val_replica is not None
                    else max_val_replica
                ),
...
def enviar_alertas_si_aplica(
    ...
    if len(servidores_en_alerta) > 0:
        email_subject = (
            "[analiticastetl][Advertencia] Delta de valor superado por la replica"
        )
        email_body = EMAIL_BODY.format(
            json_data=json.dumps(servidores_en_alerta, indent=3)
        )
        email_to = (
            [
        ...
        context.log.info(
            f"Enviado alerta por correo electronico a {email_to} con el siguiente body: {email_body}"
        )
    else:
...
comprobar_sinc_replicas_job = comprobar_sinc_replicas_graph.to_job(
    name="comprobar_sinc_replicas_job", executor_def=in_process_executor
)
