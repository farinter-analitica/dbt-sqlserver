...
@dg.op(
    ins={
        "BI_SAP_Agr_SocAlmArt_Stock_Plan": dg.In(
...
@dg.op(
    ins={
        "BI_SAP_Agr_SocAlmArtGpoCli_Demanda_Plan": dg.In(
            dagster_type=dg.Nothing,
        ),
    },
    out={
...
def limpiar_demanda_graph(
    ...
    return save_demanda_procesada(demanda_procesada_mddme, start_time)


BI_SAP_Hecho_SocAlmArtGpoCli_Demanda_Limpia = dg.AssetsDefinition.from_graph(
    graph_def=limpiar_demanda_graph,
    keys_by_input_name={
        "BI_SAP_Agr_SocAlmArtGpoCli_Demanda_Plan": dg.AssetKey(
            ["BI_FARINTER", "dbo", "BI_SAP_Agr_SocAlmArtGpoCli_Demanda_Plan"]
        ),
        "BI_SAP_Agr_SocAlmArt_Stock_Plan": dg.AssetKey(
