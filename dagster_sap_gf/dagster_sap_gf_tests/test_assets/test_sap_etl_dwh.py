...
from dagster_sap_gf.assets.sap_etl_dwh import (
    DL_SAP_T001,
...
class Test_DL_SAP_T001:
    ...
    def setup_mocks(self, mocker):
        self.mocked_dwh_farinter_dl = mocker.patch.object(
            sql_server_resources, "dwh_farinter_dl", autospec=True
        )
    ...
    def test_error_on_query_date(self, setup_mocks):
        ...
        DL_SAP_T001(
            context=self.mock_context, dwh_farinter_dl=self.mocked_dwh_farinter_dl
        )

        self.log_error_spy.assert_called_once()
        self.mocked_dwh_farinter_dl.get_connection.assert_called_once()
        assert self.log_error_spy.called and self.log_error_spy.call_args[0][
            0
        ].startswith("Error converting date:"), (
            "Bad query return value not logged correctly."
        )
