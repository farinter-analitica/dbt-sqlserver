...
- Pre-commit ejecuta chequeos estáticos y formatea el código.
...
- **Dagster Dev**: Inicia la UI de Dagster con `uv run dagster dev -h "0.0.0.0"` y accede en http://localhost:3000.
...
## Patrones y Ejemplos
- **Gestión de Llaves SSH**: Usa `scripts/deployment.py setup-deploy-key --repo=<repo> --org=<org>` para gestionar llaves de despliegue de dependencias privadas.
...
- **Ejecución de Comandos con uv**:
  ...
  - **DBT**: El directorio `dbt_dwh_farinter` contiene modelos y scripts DBT para operaciones de data warehouse. Usa `uv run --env-file .env dbt_dwh_farinter dbt <comando> --project-dir dbt_dwh_farinter ` para ejecutar comandos DBT.
