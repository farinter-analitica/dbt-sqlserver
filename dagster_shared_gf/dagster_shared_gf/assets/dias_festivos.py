...
def obtener_dias_festivos(codigos_pais, fecha_inicio, fecha_fin) -> pl.DataFrame:
    ...
    for codigo_pais in codigos_pais:
        ...
        if respuesta.status_code == 200:
            ...
            print(f"Error al obtener datos para {codigo_pais}: {respuesta.status_code} {respuesta.text}")
            raise Exception(f"Error al obtener datos para {codigo_pais}: {respuesta.status_code} {respuesta.text}")
...
def web_api_DL_Edit_CalendarioNoLaboral(context: AssetExecutionContext, dwh_farinter_dl: SQLServerResource):
    ...
    with dwh_farinter_dl.get_connection(engine="sqlalchemy") as conn:
        df_festivos.write_database(table_name=f"{schema}.{table}", connection=conn, if_table_exists="replace")
    context.log.info(f"Guardado {df_festivos.shape[0]} registros en la tabla {schema}.{table}")
...
if __name__ == "__main__":
    codigos_pais = ["HN", "NI", "CR", "SV", "GT"]  # Lista de códigos ISO2 de los países
    fecha_inicio = (datetime.now().replace(day=1, month=1)).strftime('%Y-%m-%d')  # Formato YYYY-MM-DD
    fecha_fin = (datetime.now() + timedelta(days=365)).strftime('%Y-%m-%d')     # Formato YYYY-MM-DD
