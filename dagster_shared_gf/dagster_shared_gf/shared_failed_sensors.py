...
def get_asset_owners(
    asset_key: AssetKey, context: SensorEvaluationContext
) -> list[str]:
    """
    Fetch asset owners from context. This simulates looking up asset metadata.
    Uses context to fetch dynamic data for owners.
    """
    asset_metadata = context.repository_def.assets_defs_by_key.get(
        asset_key
    ).owners_by_key.get(asset_key)
    return asset_metadata if asset_metadata else []


def get_downstream_lineage_with_owners(
    asset_key: AssetKey, job: JobDefinition, context: SensorEvaluationContext
) -> dict[AssetKey, list[str]]:
    """
...
def create_email_body(
    asset_key: AssetKey, downstream_owners: dict[AssetKey, list[str]]
):
    """
...
def failed_asset_notification_sensor(
    ...
    if context.cursor:
        ...
        event_datetime = (
            datetime.fromtimestamp(event_datetime, tz=timezone.utc)
            if event_datetime
            else None
        )
    else:
    ...
    for event in events:
        ...
        if not event.asset_key:
            ...
            failed_asset_key = (
                failed_asset_key_list.pop() if failed_asset_key_list else None
            )
            if not failed_asset_key:
                ...
                failed_asset_key_list = [
                    assetkey
                    for handle, assetkey in input_node_handles.items()
            ...
            if not failed_asset_key:
                ...
                failed_asset_key_list = [
                    assetkey
                    for handle, assetkey in output_node_handles.items()
                    if handle.node_handle == node_handle
                ]
                failed_asset_key = (
                    failed_asset_key_list[0] if failed_asset_key_list else None
...
if __name__ == "__main__":
    ...
    with instance_for_test() as instance:
        ...
        def can_fail_asset_check(context: AssetCheckExecutionContext):
            ...
            if status == "failed":
                context.log.error(
                    f"Asset {asset_key} reported failure status in metadata."
                )
                return AssetCheckResult(
                    passed=False,
                ...
                # Asset is successful
                context.log.debug(
                    f"Asset {asset_key} reported success status in metadata."
                )
        ...
        can_fail_job = define_asset_job(
            name="can_fail_job",
            selection=AssetSelection.assets(can_fail_asset).downstream(),
        )
        ...
        class EmailResource:
            ...
            def send_email(
                ...
                email_subject,
                email_body,
                context: SensorEvaluationContext = context,
            ):
        ...
        context.log.debug("Iniciando prueba de notificación de fallos")
        # Replace original function to use context

        # Mock instance to return fake event logs
        def return_fake_event_logs(
            *args, context: SensorEvaluationContext = context, **kwargs
        ):
            event_records_filter: EventRecordsFilter = kwargs.get(
            ...
            if (
                not event_records_filter.after_cursor
        ...
        def functional_test_events():
            ...
            context.instance.get_event_records = MagicMock(
                side_effect=get_event_records_backup, spec=get_event_records_backup
            )

            context.repository_def.get_job("can_fail_job").execute_in_process(
                instance=instance
            )
            context.update_cursor(
                failed_asset_notification_sensor(
                    context, enviador_correo_e_analitica_farinter=EmailResource()
                ).cursor
            )

            assert context.instance.get_event_records.call_count == 1, (
                f"Expected 1 calls accumulated to get_event_records but got {context.instance.get_event_records.call_count}"
            )
            assert send_email_mock.call_count == 0, (
                f"send_email_mock.call_count {send_email_mock.call_count} Expected 0 calls to send_email"
            )
            ...
            try:
                context.repository_def.get_job("can_fail_job").execute_in_process(
                    ...
                    run_config={
                        "ops": {
                            can_fail_asset.get_asset_spec().key.to_python_identifier(): {
                                "config": {"fail": True}
                            }
                        }
                    },
            ...
            assert context.instance.get_event_records.call_count == 2, (
                f"Expected 2 calls accumulated to get_event_records but got {context.instance.get_event_records.call_count}"
            )
            assert send_email_mock.call_count == 1, (
                f"send_email_mock.call_count {send_email_mock.call_count} Expected 1 calls to send_email"
            )
            ...
            assert context.instance.get_event_records.call_count == 4, (
                "Expected 4 calls accumulated to get_event_records"
            )
            assert send_email_mock.call_count == 1, (
                f"send_email_mock.call_count {send_email_mock.call_count} Expected 1 calls to send_email"
            )
        ...
        def unit_test_send_email():
            send_email_mock.reset_mock()
            context.instance.get_event_records = MagicMock(
                side_effect=return_fake_event_logs, spec=get_event_records_backup
            )
            ...
            assert context.instance.get_event_records.call_count == 1, (
                f"Expected 1 call to get_event_records but got {context.instance.get_event_records.call_count}"
            )
            assert send_email_mock.call_count == 1, (
                f"Expected 1 call to send_email but got {send_email_mock.call_count}"
            )
            ...
            assert context.instance.get_event_records.call_count == 3, (
                f"Expected 3 calls accumulated to get_event_records but got {context.instance.get_event_records.call_count}"
            )
            assert send_email_mock.call_count == 1, (
                f"send_email_mock.call_count {send_email_mock.call_count} Expected no more calls to send_email"
            )
