...
final as (
    ...
    SELECT
         ...
         '' AS [Forma Pago]
    FROM base
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatSucursal -- {{ source('AN_FARINTER', 'AN_Cal_ClientesCategorias_Kielsa') }}
         ON base.Pais_Id = CatSucursal.Pais_Id
         AND base.Cliente_Id = CatSucursal.Cliente_Id
         AND CatSucursal.Tipo_Id = 'Sucursal'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatSegmento
         ON base.Pais_Id = CatSegmento.Pais_Id
         AND base.Cliente_Id = CatSegmento.Cliente_Id
         AND CatSegmento.Tipo_Id = 'Segmento'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatHorario
         ON base.Pais_Id = CatHorario.Pais_Id
         AND base.Cliente_Id = CatHorario.Cliente_Id
         AND CatHorario.Tipo_Id = 'Horario'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatDiaMes
         ON base.Pais_Id = CatDiaMes.Pais_Id
         AND base.Cliente_Id = CatDiaMes.Cliente_Id
         AND CatDiaMes.Tipo_Id = 'Dia_Mes'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatDiaSemana
         ON base.Pais_Id = CatDiaSemana.Pais_Id
         AND base.Cliente_Id = CatDiaSemana.Cliente_Id
         AND CatDiaSemana.Tipo_Id = 'Dia_Semana'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatCanal
         ON base.Pais_Id = CatCanal.Pais_Id
         AND base.Cliente_Id = CatCanal.Cliente_Id
         AND CatCanal.Tipo_Id = 'Canal'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatDepto_Art
         ON base.Pais_Id = CatDepto_Art.Pais_Id
         AND base.Cliente_Id = CatDepto_Art.Cliente_Id
         AND CatDepto_Art.Tipo_Id = 'Depto_Art'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatEnfermedades
         ON base.Pais_Id = CatEnfermedades.Pais_Id
         AND base.Cliente_Id = CatEnfermedades.Cliente_Id
         AND CatEnfermedades.Tipo_Id = 'Enfermedades'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatCanje
         ON base.Pais_Id = CatCanje.Pais_Id
         AND base.Cliente_Id = CatCanje.Cliente_Id
         AND CatCanje.Tipo_Id = 'Canje'
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesCategorias_Kielsa CatAseguradora
         ON base.Pais_Id = CatAseguradora.Pais_Id
         AND base.Cliente_Id = CatAseguradora.Cliente_Id
         AND CatAseguradora.Tipo_Id = 'Aseguradora'
    LEFT JOIN BI_FARINTER.dbo.BI_Dim_Sucursal_Kielsa Sucursal -- {{ source('BI_FARINTER', 'BI_Dim_Sucursal_Kielsa') }}
         ON CatSucursal.Categoria = Sucursal.Sucursal_Id
    LEFT JOIN BI_FARINTER.dbo.BI_Dim_TipoSucursal_Kielsa TipoSucursal -- {{ source('BI_FARINTER', 'BI_Dim_TipoSucursal_Kielsa') }}
         ON CatSegmento.Categoria = TipoSucursal.TipoSucursal_Id
    LEFT JOIN BI_FARINTER.dbo.BI_Dim_Dias Dias -- {{ source('BI_FARINTER', 'BI_Dim_Dias') }}
         ON CONVERT(INT, CatDiaSemana.Categoria) = Dias.Dias_Id
    LEFT JOIN BI_FARINTER.dbo.BI_Dim_CanalVenta_Kielsa Canal -- {{ source('BI_FARINTER', 'BI_Dim_CanalVenta_Kielsa') }}
         ON CONVERT(INT, CatCanal.Categoria) = Canal.CanalVenta_Id
    LEFT JOIN BI_FARINTER.dbo.BI_Dim_DeptoArt_Kielsa DeptoArt -- {{ source('BI_FARINTER', 'BI_Dim_DeptoArt_Kielsa') }}
         ON CatDepto_Art.Categoria = DeptoArt.DeptoArt_Id
    LEFT JOIN BI_FARINTER.dbo.BI_Dim_TipoCliente_Kielsa TipoCliente -- {{ source('BI_FARINTER', 'BI_Dim_TipoCliente_Kielsa') }}
         ON CatAseguradora.Categoria = TipoCliente.TipoCliente_Id
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesMetricas_Kielsa CMETRIC -- {{ source('AN_FARINTER', 'AN_Cal_ClientesMetricas_Kielsa') }}
         ON base.Pais_Id = CMETRIC.Pais_Id AND base.Cliente_Id = CMETRIC.Cliente_Id
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesEstadisticas_Kielsa CESTADIST -- {{ source('AN_FARINTER', 'AN_Cal_ClientesEstadisticas_Kielsa') }}
         ON base.Pais_Id = CESTADIST.Pais_Id AND base.Cliente_Id = CESTADIST.Cliente_Id
    LEFT JOIN AN_FARINTER.dbo.AN_Cal_ClientesEstados_Kielsa CESTADOS -- {{ source('AN_FARINTER', 'AN_Cal_ClientesEstados_Kielsa') }}
         ON base.Pais_Id = CESTADOS.Pais_Id AND base.Cliente_Id = CESTADOS.Cliente_Id
    LEFT JOIN (
         SELECT EMP_ID_LDCOM, Identidad
         FROM CRM_FARINTER.dbo.CRM_Kielsa_CarritoSeguimiento --{{ source('CRM_FARINTER', 'CRM_Kielsa_CarritoSeguimiento') }}
         WHERE Indicador_Carrito_Cerrado = 0
           AND NOT EXISTS (
               SELECT 1
               FROM CRM_FARINTER.dbo.CAMPAÑA C --{{ source('CRM_FARINTER', 'CAMPANA') }}
               INNER JOIN CRM_FARINTER.dbo.CLIENTE_X_LISTA CL --{{ source('CRM_FARINTER', 'CLIENTE_X_LISTA') }}
                    ON CL.LISTID = C.LISTA_ID
               WHERE GETDATE() BETWEEN C.FECHA_INICIO AND C.FECHA_FINAL
         ...
         ON base.Pais_Id = CARSEG.EMP_ID_LDCOM
         AND base.Cliente_Id = CARSEG.Identidad COLLATE DATABASE_DEFAULT
)
select *
 from final