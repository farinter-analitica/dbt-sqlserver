...
(
    SELECT FP.MonederoTarj_Id_Limpio AS Monedero_Id
        , FP.Emp_Id 
		...
		,  SUM(CASE WHEN FP.Factura_Fecha BETWEEN DATEADD(DAY, -30, GETDATE()) AND GETDATE() 
					THEN FP.Cantidad_Padre 
						ELSE 0 
				END) AS Cantidad_30
		, SUM(CASE WHEN FP.Factura_Fecha BETWEEN DATEADD(DAY, -60, GETDATE()) AND GETDATE() 
					THEN FP.Cantidad_Padre 
						ELSE 0 
				END) AS Cantidad_60
		, SUM(CASE WHEN FP.Factura_Fecha BETWEEN DATEADD(DAY, -90, GETDATE()) AND GETDATE() 
					THEN FP.Cantidad_Padre 
						ELSE 0 
				END) AS Cantidad_90
		, SUM(CASE WHEN FP.Factura_Fecha BETWEEN DATEADD(DAY, -120, GETDATE()) AND GETDATE() 
					THEN FP.Cantidad_Padre 
						ELSE 0 
				END) AS Cantidad_120
		, SUM(CASE WHEN FP.Factura_Fecha BETWEEN DATEADD(DAY, -180, GETDATE()) AND GETDATE() 
					THEN FP.Cantidad_Padre 
						ELSE 0 
				END) AS Cantidad_180
