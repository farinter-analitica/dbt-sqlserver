...
{% if is_incremental() and last_date != '19000101' %} 
    WITH Resumen_Fecha_Desde AS (
        SELECT
            FP.AnioMes_Id,
            CAST(MIN(FP.Factura_Fecha) AS DATE) AS Fecha_Desde
        FROM {{ source ('DL_FARINTER', 'DL_Kielsa_FacturasPosiciones') }} AS FP
        WHERE
            FP.DWH_Fecha_Actualizado >= '{{ last_date }}'
...
SELECT
    ...
    FE.[MonederoTarj_Id],
    FE.Monedero_Id AS [MonederoTarj_Id_Limpio],
    FE.[Monedero_Id],
    FE.[Cliente_Id],
    FE.[Vendedor_Id],
    FE.[Factura_Estado],
    ...
    FE.HashStr_SucEmp,
    FE.HashStr_SucBodEmp, --borrar cuando sea posible
    FE.HashStr_EmplEmp,
    C.HashStr_CasaEmp, --borrar cuando sea posible
    SUC.Hash_SucursalEmp,
    DOC.Hash_SubDocEmp,
    MON.Hash_MonederoEmp,
    ISNULL(FE.[Factura_Fecha], '19000101') AS Factura_Fecha,
    DATEPART(HOUR, FE.[Factura_FechaHora]) AS Hora_Id,
    ...
    ISNULL(FE.[TipoDoc_Id], 0) AS [TipoDoc_Id], --borrar cuando sea posible
    ISNULL(FE.[Factura_Id], 0) AS [Factura_Id], --borrar cuando sea posible
    ISNULL(FP.[Articulo_Id], 0) AS [Articulo_Id], --borrar cuando sea posible
    CAST(CAST(CLI.HashStr_CliEmp AS VARBINARY) AS BIGINT) AS Hash_ClienteEmp, --borrar cuando sea posible
    ...
    {{ dwh_farinter_concat_key_columns(columns=['Emp_Id', 'Suc_Id', 'TipoDoc_Id', 'Caja_Id', 'Factura_Id'], input_length=49, table_alias='FE') }} AS [EmpSucDocCajFac_Id]
    {% if is_incremental() and last_date != '19000101' %} 
...
{% if is_incremental() and last_date != '19000101' %}
    INNER JOIN Resumen_Fecha_Desde AS Res
        ON
            FE.AnioMes_Id = Res.AnioMes_Id
...
INNER JOIN {{ source ('BI_FARINTER', 'BI_Dim_Calendario') }} AS CAL --Optimizacion por tema de particiones
    ON CAL.Fecha_Id = FE.Factura_Fecha AND CAL.AnioMes_Id = FE.AnioMes_Id
LEFT JOIN {{ ref ('BI_Kielsa_Dim_Monedero') }} AS MON
    ON
        MON.Monedero_Id = FE.Monedero_Id
...
LEFT JOIN (
    SELECT
        ...
        AnioMes_Id,
        Articulo_Id,
        SUM(Descuento_Porcentaje) AS Descuento_Porcentaje
    FROM {{ source ('DL_FARINTER', 'DL_Kielsa_FacturaPosicionDescuento') }}
    WHERE Descuento_Origen = 6
    GROUP BY AnioMes_Id, Emp_Id, TipoDoc_Id, Suc_Id, Caja_Id, Factura_Id, Articulo_Id
) AS FPD
    ON
        FPD.Emp_Id = FP.Emp_Id
...
LEFT JOIN (
    SELECT
        ...
        AnioMes_Id,
        Articulo_Id,
        SUM(Descuento_Monto) AS Valor_Descuento_Tercera_Edad
    FROM {{ source ('DL_FARINTER', 'DL_Kielsa_FacturaPosicionDescuento') }}
    WHERE Descuento_Origen = 1 AND Descuento_Nombre LIKE 'TERCERA%EDAD%'
    GROUP BY AnioMes_Id, Emp_Id, TipoDoc_Id, Suc_Id, Caja_Id, Factura_Id, Articulo_Id
) AS DTE
    ON
        DTE.Emp_Id = FP.Emp_Id
