...
(
	...
	SELECT M.Monedero_Id, M.Emp_Id
	FROM {{ref("BI_Kielsa_Dim_Monedero")}} M
)
	SELECT ISNULL(M.Monedero_Id,'') Monedero_Id     
		...
		, ISNULL(CASE 
			...
			ELSE DATEDIFF(DAY, MUM.Fecha_Primera_Factura, FM.Fecha_Max) / 
	...
	LEFT JOIN {{ ref('BI_Kielsa_Agr_Monedero_Metricas_Ventana') }} MUM
		ON MUM.Monedero_Id = M.Monedero_Id
