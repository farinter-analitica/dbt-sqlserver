...
Alertas AS (
    SELECT
        MAX(Alerta.Alerta_Id) AS PV_Alerta_Id,
        Alerta.Articulo_Id AS Articulo_Id,
        Alerta.Emp_Id
...
Ingresos AS (
    ...
    {% if is_incremental() -%}        WHERE Ingresos.Fecha_Id > '{{ last_date }}'
    {% else -%}
...
ComprasLocales AS (
    SELECT
        ...
        MAX(CASE Art.ABC_Cadena
            WHEN 'A' THEN 1 WHEN 'B' THEN 2 WHEN 'C' THEN 3 WHEN 'D' THEN 4 WHEN 'E' THEN 5 ELSE
                0
        END) AS ABCCadena_Id,
        ...
        MAX(CASE
            WHEN
                YEAR(Estado.Fecha_Id) = YEAR(GETDATE())
    ...
    INNER JOIN DL_FARINTER.dbo.DL_Kielsa_Orden_Local_Encabezado OrdenEnc --{{ source('DL_FARINTER', 'DL_Kielsa_Orden_Local_Encabezado') }}
        ON
            OrdenDetalle.Emp_Id = OrdenEnc.Emp_Id
    ...
    LEFT OUTER JOIN DL_FARINTER.[dbo].[DL_TC_ArticuloXMecanica_Kielsa] ArtxMec --{{ ref('DL_TC_ArticuloXMecanica_Kielsa') }}
        ON
            OrdenDetalle.Emp_Id = ArtxMec.Emp_Id
    ...
    LEFT JOIN AN_FARINTER.[dbo].[AN_Cal_ArticulosEstado_Kielsa] Estado --{{ source('AN_FARINTER', 'AN_Cal_ArticulosEstado_Kielsa') }}
        ON
            OrdenDetalle.Emp_Id = Estado.Pais_Id
    ...
    LEFT OUTER JOIN Ingresos Ingresos
        ON
            OrdenDetalle.Emp_Id = Ingresos.Emp_Id
...
,
comprasGenerales AS (
    SELECT * FROM ComprasLocales
    UNION ALL
    SELECT * FROM ComprasExteriores
)
