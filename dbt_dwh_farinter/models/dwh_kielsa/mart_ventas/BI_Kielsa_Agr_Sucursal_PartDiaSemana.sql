...
SELECT 
    ...
    {%- for field in metric_fields %}
    CAST(Sum_{{ field }} / Semanas_Muestra AS DECIMAL(16,6)) AS Prom_{{ field }},
    ISNULL(CAST(Sum_{{ field }} / NULLIF(SUM(Sum_{{ field }}) 
        OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,12)),0)  AS Part_{{ field }}{% if not loop.last %},{% endif %}
    {%- endfor %}        
--INTO #Temp
FROM ResumenBase
)
SELECT *,
    {%- for field in metric_fields %}
    Part_{{ field }}*7 AS Peso_{{ field }}{% if not loop.last %},{% endif %}
    {%- endfor %}        
FROM Metricas
