...
SELECT 
    ...
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Cantidad_Articulos / NULLIF(SUM(Sum_Cantidad_Articulos) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Cantidad_Articulos,
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1 
    ...
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Conteo_Trx_Es_Tercera_Edad / NULLIF(SUM(Sum_Conteo_Trx_Es_Tercera_Edad) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Conteo_Trx_Es_Tercera_Edad,
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Conteo_Trx_Es_Asegurado / NULLIF(SUM(Sum_Conteo_Trx_Es_Asegurado) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Conteo_Trx_Es_Asegurado,
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Conteo_Trx_Acumula_Monedero / NULLIF(SUM(Sum_Conteo_Trx_Acumula_Monedero) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Conteo_Trx_Acumula_Monedero,
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Conteo_Trx_Contiene_Farma / NULLIF(SUM(Sum_Conteo_Trx_Contiene_Farma) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Conteo_Trx_Contiene_Farma,
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Cantidad_Unidades_Relativa / NULLIF(SUM(Sum_Cantidad_Unidades_Relativa) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Cantidad_Unidades_Relativa,
    ISNULL(CAST(CASE WHEN Es_Sucursal_Semanas_Completas = 1
        THEN Sum_Segundos_Transaccion_Estimado / NULLIF(SUM(Sum_Segundos_Transaccion_Estimado) OVER(PARTITION BY Emp_Id, Suc_Id),0) ELSE 1/52 END AS DECIMAL(16,12)),0) AS Part_Segundos_Transaccion_Estimado
FROM CompletarLlave
)
SELECT *,
    Part_Cantidad_Padre*52 AS Peso_Cantidad_Padre,
    Part_Cantidad_Articulos*52 AS Peso_Cantidad_Articulos,
    ...
    Part_Conteo_Trx_Es_Tercera_Edad*52 AS Peso_Conteo_Trx_Es_Tercera_Edad,
    Part_Conteo_Trx_Es_Asegurado*52 AS Peso_Conteo_Trx_Es_Asegurado,
    Part_Conteo_Trx_Acumula_Monedero*52 AS Peso_Conteo_Trx_Acumula_Monedero,
    Part_Conteo_Trx_Contiene_Farma*52 AS Peso_Conteo_Trx_Contiene_Farma,
    Part_Cantidad_Unidades_Relativa*52 AS Peso_Cantidad_Unidades_Relativa,
    Part_Segundos_Transaccion_Estimado*52 AS Peso_Segundos_Transaccion_Estimado
FROM Metricas