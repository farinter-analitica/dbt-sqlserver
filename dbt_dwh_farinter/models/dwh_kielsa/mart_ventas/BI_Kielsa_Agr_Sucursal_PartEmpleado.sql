...
SELECT 
    ...
    ISNULL(CAST(Sum_Valor_Descuento_Financiero / NULLIF(SUM(Sum_Valor_Descuento_Financiero) OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,4)),0)  AS Part_Valor_Descuento_Financiero,
    ISNULL(CAST(Sum_Valor_Acum_Monedero / NULLIF(SUM(Sum_Valor_Acum_Monedero) OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,4)),0)  AS Part_Valor_Acum_Monedero,
    ISNULL(CAST(Sum_Valor_Descuento_Cupon / NULLIF(SUM(Sum_Valor_Descuento_Cupon) OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,4)),0)  AS Part_Valor_Descuento_Cupon,
    ISNULL(CAST(Sum_Descuento_Proveedor / NULLIF(SUM(Sum_Descuento_Proveedor) OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,4)),0)  AS Part_Valor_Descuento_Proveedor,
    ISNULL(CAST(Sum_Valor_Descuento_Tercera_Edad / NULLIF(SUM(Sum_Valor_Descuento_Tercera_Edad) OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,4)),0)  AS Part_Valor_Descuento_Tercera_Edad,
    ISNULL(CAST(Sum_Conteo_Transacciones / NULLIF(SUM(Sum_Conteo_Transacciones) OVER(PARTITION BY Emp_Id, Suc_Id),0) AS DECIMAL(16,4)),0)  AS Part_Conteo_Transacciones
--INTO #Temp
FROM ResumenBase
