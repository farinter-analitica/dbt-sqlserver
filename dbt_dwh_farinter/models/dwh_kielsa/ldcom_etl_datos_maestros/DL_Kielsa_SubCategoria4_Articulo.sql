...
{%- set unique_key_list = ["SubCategoria4Art_Id","SubCategoria3Art_Id","SubCategoria2Art_Id","SubCategoria1Art_Id","CategoriaArt_Id","Emp_Id"] -%}
{{ 
    config(
...
{%- if not loop.first %}
	...
	SELECT ISNULL({{item['Empresa_Id']}},0) AS [Emp_Id]
		...
		, ABS(CAST(CAST(HASHBYTES('SHA2_256', CONCAT(SubCategoria4_Id, '-', SubCategoria3_Id, '-', SubCategoria2_Id, '-', SubCategoria_Id, '-', Categoria_Id, '-', Emp_Id)) AS INT) AS bigint))  AS Hash_CatSubCat1_2_3_4Emp 
