...
usuario_jop AS (
	SELECT RJ.Emp_Id,
		...
		U.Usuario_Id
	FROM rol_jop RJ
		INNER JOIN DL_FARINTER.DBO.DL_Kielsa_Seg_Usuario_x_Rol UR -- {{ ref("DL_Kielsa_Seg_Usuario_x_Rol") }}
		ON RJ.Rol_Id = UR.Rol_Id
		AND RJ.Emp_Id = UR.Emp_Id
		INNER JOIN DL_FARINTER.DBO.DL_Kielsa_Seg_Usuario U -- {{ ref("DL_Kielsa_Seg_Usuario") }}
		ON U.Emp_Id = UR.Emp_Id
...
usuario_supervisor AS (
	SELECT RS.Emp_Id,
		...
		U.Usuario_Nombre,
		U.Usuario_Id
	FROM rol_supervisor RS
		INNER JOIN DL_FARINTER.DBO.DL_Kielsa_Seg_Usuario_x_Rol UR -- {{ ref("DL_Kielsa_Seg_Usuario_x_Rol") }}
		ON RS.Rol_Id = UR.Rol_Id
		AND RS.Emp_Id = UR.Emp_Id
		INNER JOIN DL_FARINTER.DBO.DL_Kielsa_Seg_Usuario U ON U.Emp_Id = UR.Emp_Id
		AND U.Usuario_Id = UR.Usuario_Id
	WHERE U.Usuario_Eliminado = 0
		AND U.Usuario_Cuenta_Deshabilitada = 0 
),
usuario_supervisor_sucursal AS (
	...
	FROM usuario_supervisor US
		INNER JOIN DL_FARINTER.DBO.DL_Kielsa_Seg_Sucursal_x_Usuario SU -- {{ ref('DL_Kielsa_Seg_Sucursal_x_Usuario') }}
		ON us.Emp_Id = SU.Emp_id
