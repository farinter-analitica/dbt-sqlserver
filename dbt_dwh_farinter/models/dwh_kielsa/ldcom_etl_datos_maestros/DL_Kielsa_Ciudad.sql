...
{%- set unique_key_list = ["Ciudad_Id","Municipio_Id","Departamento_Id","Emp_Id"] -%}
{{ 
    config(
...
{%- if not loop.first %}
	...
	SELECT ISNULL({{item['Empresa_Id']}},0) AS [Emp_Id]
		...
		, ABS(CAST(CAST(HASHBYTES('SHA2_256', CONCAT(Nivel3_Id, '-', Nivel2_Id, '-', Nivel1_Id, '-', Emp_Id)) AS INT) AS bigint))  AS Hash_DeptoMunCiudadEmp 
