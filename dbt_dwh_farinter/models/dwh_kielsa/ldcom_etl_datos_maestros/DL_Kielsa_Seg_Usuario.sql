...
{%- if is_incremental() %}
    {%- set last_date = run_single_value_query_on_relation_and_return(query="""select ISNULL(CONVERT(VARCHAR,DATEADD(DAY, -7, max(Usuario_Fec_Actualizacion)), 112), '19000101')  from  """ ~ this, relation_not_found_value='19000101'|string)|string %}
{%- else %}
...
{%- if not loop.first %}
	...
	{%- if is_incremental() %}
	AND Usuario_Fec_Actualizacion >= {{last_date}}
	{%- endif %}
