...
{# Returns: [{Empresa_Id,Emp_Id_Original,Pais_Id,LS_LDCOM_Replica,D_LDCOM_Replica}] #}

{%- if is_incremental() %}
    {%- set last_date = run_single_value_query_on_relation_and_return(
		query="""select ISNULL(CONVERT(VARCHAR,DATEADD(DAY, -365, max(Usuario_Fec_Actualizacion)), 112), '19000101') as fecha_a from  """ ~ this,
...
{%- for item in valid_empresas -%}
        ...
        {%- if is_incremental() %}
        --WHERE Usuario_Fec_Actualizacion >= '{{ last_date }}' --LD NO ES FIABLE
        {%- endif %}
