...
Final AS (
    SELECT -- noqa: ST06
        ...
        COALESCE(UI.Rol_Id_Mapeado, 0) AS Rol_Id_Mapeado,
        COALESCE(UI.Rol_Nombre_Mapeado, 'Sin_Rol') AS Rol_Nombre_Mapeado,
        ROL.Rol_Jerarquia,
        UI.Vendedor_Id,
    ...
    FROM SucursalUsuario AS SU
    INNER JOIN UsuarioInfo AS UI
        ON
            SU.Usuario_Id = UI.Usuario_Id
            AND SU.Emp_Id = UI.Emp_Id
    LEFT JOIN RolAsignado AS RA
        ON
            SU.Usuario_Id = RA.Usuario_Id
            AND SU.Suc_Id = RA.Suc_Id
            AND SU.Emp_Id = RA.Emp_Id
    LEFT JOIN Roles AS ROL
        ON
            SU.Emp_Id = ROL.Emp_Id
...
SELECT
    *,
    ...
    {{ dwh_farinter_concat_key_columns(columns=["Emp_Id","Suc_Id","Usuario_Id"], input_length=99) }} AS [EmpSucUsu_Id],
