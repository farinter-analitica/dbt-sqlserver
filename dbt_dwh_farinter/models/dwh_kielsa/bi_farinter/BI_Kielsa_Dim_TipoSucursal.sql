{%- set unique_key_list = ["TipoSucursal_Id", "Emp_Id"] -%}
...
WITH Dim_TipoSucursal AS (
    SELECT --noqa: ST06
        ISNULL(TipoSucursal_Id, 0) AS [TipoSucursal_Id],
        ISNULL(Emp_Id, 0) AS [Emp_Id],
        ISNULL(TipoSucursal_Nombre, 'N.D.') AS [TipoSucursal_Nombre],
        Fecha_Actualizado AS Fecha_Carga,
        Fecha_Actualizado
    FROM {{ ref('DL_Kielsa_TipoSucursal') }}
    {%- if is_incremental() %}
...
SELECT
    TipoSucursal_Id,
    Emp_Id,
    {{ dwh_farinter_concat_key_columns(columns=['Emp_Id', 'TipoSucursal_Id'], input_length=50) }} AS [EmpTipoSucursal_Id],
    TipoSucursal_Nombre,
    Fecha_Carga,
    Fecha_Actualizado
FROM Dim_TipoSucursal
