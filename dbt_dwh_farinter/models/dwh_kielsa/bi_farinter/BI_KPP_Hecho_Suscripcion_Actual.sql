...
SELECT --noqa: ST06
    ...
    */
    ...
    CASE
        WHEN
            C.Ingreso BETWEEN DATEADD(DAY, -1, A.FRegistro)
...
INNER JOIN [BI_Kielsa_Dim_Monedero] AS C  -- {{ ref('BI_Kielsa_Dim_Monedero') }}
    ON
        A.Identidad_Limpia = C.Monedero_Id
        AND C.Emp_Id = 1
LEFT JOIN (
...
INNER JOIN DL_FARINTER.dbo.DL_Kielsa_Sucursal AS F -- {{ source('DL_FARINTER', 'DL_Kielsa_Sucursal') }}
    ON
        A.Sucursal_Registro = F.Sucursal_Id
        AND C.Emp_Id = F.Emp_Id
LEFT JOIN {{ ref("BI_Kielsa_Dim_Usuario") }} AS U
    ON
        A.Usuario_Registro = U.Usuario_Login COLLATE DATABASE_DEFAULT
        AND U.Emp_Id = 1
WHERE
    C.Emp_Id = 1 AND (A.Indicador_Borrado <> 1 OR A.Indicador_Borrado IS NULL)
    {%- if is_incremental() %}
