...
{%- if is_incremental() %}
    {%- set last_date = run_single_value_query_on_relation_and_return(query="""select ISNULL(CONVERT(VARCHAR,DATEADD(DAY, -15, max(Fecha_Consulta)), 112), '19000101') as fecha_a from  """ ~ this, relation_not_found_value='19000101'|string)|string %}
{%- else %}
...
SELECT
    ISNULL({{ dwh_farinter_hash_column( columns = ["Video_Id", "Consultado_Id"], table_alias="A") }}, '') AS Consulta_Id, --ISNULL(COALESCE(VC._id_oid, CO._id_oid, PV._id_oid),'') AS _Id
    *,
