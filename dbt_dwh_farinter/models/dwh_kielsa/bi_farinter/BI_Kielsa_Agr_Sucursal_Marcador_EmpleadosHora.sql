...
WHERE Pais_Id =1 and Sucursal_Id IS NOT NULL AND Es_Valido = 1
{% if is_incremental() %}
...
Calendario AS
(
	SELECT Fecha_Calendario, Semana_del_Anio_ISO, Dia_de_la_Semana -- este calendario ya trae semana y dia iso
...
(
	...
	CROSS JOIN Calendario
		WHERE Marcador_Base.Fecha_Entrada <= Calendario.Fecha_Calendario
