...
(
    ...
    SELECT 
        CAST(Suc_Id AS INT) AS Suc_Id,
        dia_id as Dia_Semana_Iso_Id, 
		H_Apertura,
		H_Cierre,
        CAST(H_Apertura AS datetime) AS FH_Apertura,
        CASE WHEN H_Apertura > H_Cierre 
			THEN DATEADD(DAY,1,CAST(H_Cierre  AS datetime))
			ELSE CAST(H_Cierre  AS datetime) END AS FH_Cierre
		, 
		CASE WHEN H_Cierre > H_Apertura THEN 0 ELSE 1 END AS Es_Cierre_Dia_Siguiente,
        activa
        -- CASE 
        ...
        -- END AS Es_24_Horas
		--select top 100 *
    FROM [DL_FARINTER].[excel].[DL_Kielsa_Horario_Temp] -- {{ source('DL_FARINTER_excel', 'DL_Kielsa_Horario_Temp') }}
