{%- set unique_key_list = ["CategoriaArt_Id", "Emp_Id"] -%}
...
WITH Dim_Categoria AS (
    SELECT --noqa: ST06
        ISNULL(Categoria_Id, 0) AS [CategoriaArt_Id],
        ISNULL(Emp_Id, 0) AS [Emp_Id],
        ISNULL(Categoria_Nombre, 'N.D.') AS [CategoriaArt_Nombre],
        Hash_CategoriaEmp,
        Fecha_Carga,
        Fecha_Actualizado
    FROM {{ ref('DL_Kielsa_Categoria_Articulo') }}
    {%- if is_incremental() %}
...
SELECT
    ...
    Emp_Id,
    CategoriaArt_Nombre,
    {{ dwh_farinter_concat_key_columns(columns=['Emp_Id', 'CategoriaArt_Id'], input_length=50) }} AS [EmpCategoriaArt_Id],
    Hash_CategoriaEmp,
    Fecha_Carga,
    Fecha_Actualizado
FROM Dim_Categoria;
