...
WITH Dim_Departamento AS (
    SELECT --noqa: ST06
        ISNULL(DeptoArt_Id, 0) AS [DeptoArt_Id],
        ISNULL(Emp_Id, 0) AS [Emp_Id],
        ISNULL(DeptoArt_Nombre, 'N.D.') AS [DeptoArt_Nombre],
        Hash_DeptoArtEmp,
        Tipo_Canal,
        Fecha_Carga,
        Fecha_Actualizado
    FROM {{ ref('DL_Kielsa_Departamento_Articulo') }}
    {%- if is_incremental() %}
...
SELECT
    ...
    Emp_Id,
    DeptoArt_Nombre,
    {{ dwh_farinter_concat_key_columns(columns=['Emp_Id', 'DeptoArt_Id'], input_length=50) }} AS [EmpDeptoArt_Id],
    Hash_DeptoArtEmp,
    Tipo_Canal,
    Fecha_Carga,
    Fecha_Actualizado
