...
{%- if is_incremental() %}
    {%- set last_date = run_single_value_query_on_relation_and_return(query="""select ISNULL(CONVERT(VARCHAR,DATEADD(DAY, -30*6, max(Fecha_Actualizado)), 112), '19000101') as fecha_a from  """ ~ this, relation_not_found_value='19000101'|string)|string %}
{%- else %}
...
WITH Marcador_Base AS (
    ...
    WHERE
        Pais = 'Honduras'  --OJO: Agregar Pais_ISO2 para cada pais
...
Marcador_Horario AS (
    ...
    INNER JOIN BI_FARINTER.dbo.BI_Kielsa_Dim_Sucursal S -- {{ ref('BI_Kielsa_Dim_Sucursal') }}
        ON
            Marcador_Base.sucursal_numero = S.Sucursal_Numero
            AND S.Emp_Id = 1
    LEFT JOIN Horarios
        --TODO: Falta el pais en la tabla de horarios
        ON
            S.Sucursal_Id = Horarios.suc_id
...
Marcador_Horario_Sospechoso AS (
    SELECT
        ...
        isnull(
            ...
            CASE
                WHEN
                    MH.fh_entrada < dateadd(HOUR, -2, MH.fh_apertura)
...
Marcador_Corregido AS (
    SELECT
        ...
        WHEN MH.sospecha_id IN (3)
            THEN CASE
                WHEN MH.fh_salida < dateadd(HOUR, 1, MH.fh_apertura)
                    THEN NULL
...
Marcador_Final AS (
    SELECT
        ...
        CASE
            WHEN MC.fh_entrada_corregida < dateadd(HOUR, -2, MC.fh_apertura)
                THEN dateadd(HOUR, -2, MC.fh_apertura)
            ELSE MC.fh_entrada_corregida
        END AS FH_Entrada_Corregida,
        CASE
            WHEN MC.fh_salida_corregida > dateadd(HOUR, 2, MC.fh_cierre)
                THEN dateadd(HOUR, 2, MC.fh_cierre)
            ELSE MC.fh_salida_corregida
        END AS FH_Salida_Corregida
...
SELECT --TOP 100 
    *,
    ...
    CASE
        WHEN
            MF.fh_entrada_corregida IS NULL
