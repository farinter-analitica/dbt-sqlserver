...
{{ 
    config(
      ...
      "{{ dwh_farinter_create_index(is_incremental=is_incremental(), columns=['Plan_Id']) }}",
		]
...
{% endif %}
  ...
  FROM {{ var('P_SQLLDSUBS_LS') }}.[KPP_DB].[dbo].[Plan_Suscripcion] -- {{ var('P_SQLLDSUBS_LS') }}.{{ source('KPP_DB', 'Plan_Suscripcion') }} 
  --{% if is_incremental() %}
