...
WITH TransactionSequence AS (
    ...
    FROM {{ ref('DL_Kielsa_KPP_Transacciones') }} t 
	{% if is_incremental() %}
    WHERE Fecha_Actualizado > '{{ last_date }}' 
	{% endif %}
),
LogValidation AS (
    SELECT TarjetaKC_Id,
...
ValidTransactions AS (
    SELECT t.*,
        ...
        CASE
            ...
            AND (
                Siguiente_Monto IS NULL
                OR Siguiente_Monto >= 0
            ) -- Si hay errores, verificamos que sean menos que las transacciones positivas previas
            AND (
                l.Total_Errores IS NULL
                OR t.ResponseSuccess LIKE '%true%'
                OR l.Total_Errores <= t.Transacciones_Positivas_Previas
            )
            AND t.ResponseSuccess NOT LIKE '%false%' THEN 1
            WHEN t.Monto_Rebajado > 0
            AND Siguiente_Monto = - t.Monto_Rebajado THEN 0
            ELSE 0
        END AS Es_Transaccion_Valida
    FROM TransactionSequence t
