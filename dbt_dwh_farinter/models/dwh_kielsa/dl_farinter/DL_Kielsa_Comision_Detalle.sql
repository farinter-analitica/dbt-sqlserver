...
(
	...
	SELECT ISNULL(CAST({{item['Empresa_Id']}} AS SMALLINT),0) AS [Emp_Id]
      ,ISNULL(CD.[Comision_Id],0) AS [Comision_Id]
      ,ISNULL(CD.[Suc_Id],	0) AS [Suc_Id]
      ,ISNULL(CD.[Articulo_Id] COLLATE DATABASE_DEFAULT,'') AS [Articulo_Id]
      ,CD.[Comision_Monto]
      ,CD.[Comision_Fec_Actualizacion]
      ,CD.[Consecutivo]
  	FROM  {{item['Servidor_Vinculado']}}.{{item['Base_Datos']}}.dbo.Comision_Detalle CD
  	JOIN {{item['Servidor_Vinculado']}}.{{item['Base_Datos']}}.dbo.Comision_Encabezado CE
  	ON CD.Emp_Id = CE.Emp_Id AND CD.Comision_Id = CE.Comision_Id
	WHERE CD.Emp_Id = {{ item['Empresa_Id_Original'] }}  
		AND (CD.Comision_Fec_Actualizacion >= '{{ last_date }}' OR CE.Comision_Fec_Actualizacion >= '{{ last_date }}')
    {%- endfor -%}   
)
SELECT *
