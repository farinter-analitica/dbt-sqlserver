...
SELECT 
    ...
    ISNULL(DL.Limpios_Para_Promedio + DL.Demanda_Negativa, 0) AS cantidad_vta_limpia,
    ISNULL(DL.Limpios_Para_Promedio, 0) AS demanda_positiva_limpia,
...
WHERE DP.Fecha_Id >= DATEADD(MONTH, -13, GETDATE()) --12 MESES COMPLETOS
--AND DP.Fecha_Id < DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) --ULTIMO MES COMPLETO
