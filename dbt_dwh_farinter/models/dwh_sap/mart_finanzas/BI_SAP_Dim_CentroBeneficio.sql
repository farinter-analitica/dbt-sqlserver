...
{% set unique_key_list = ['Centro_Beneficio_Id','Sociedad_CO'] %}
{{ 
    config(
...
(
SELECT --TOP 10 
ISNULL(CAST(A.[PRCTR] AS VARCHAR(10)),'X') COLLATE DATABASE_DEFAULT AS [Centro_Beneficio_Id] -- -- X-Centro de coste-Check:CSKS-Datatype:CHAR-Len:(10,0)
...
FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY KOKRS, PRCTR ORDER BY DATBI DESC) AS Fila 
	FROM {{ ref('DL_SAP_CEPCT') }} WHERE SPRAS = 'S') A
INNER JOIN (SELECT *, ROW_NUMBER() OVER (PARTITION BY KOKRS, PRCTR ORDER BY DATBI DESC) AS Fila 
	FROM {{ ref ('DL_SAP_CEPC') }}) B
ON A.KOKRS = B.KOKRS
AND A.PRCTR = B.PRCTR
--AND A.DATBI = B.DATBI
