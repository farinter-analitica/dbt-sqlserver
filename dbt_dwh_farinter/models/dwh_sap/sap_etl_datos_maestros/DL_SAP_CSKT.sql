{% set unique_key_list = ["KOSTL","KOKRS","SPRAS","DATBI"] %}
{{ 
    config(
...
SELECT ISNULL(CAST(A.[SPRAS] COLLATE DATABASE_DEFAULT AS VARCHAR(1)),'')  AS [SPRAS]  -- X-Clave de idioma-Check:T002-Datatype:LANG-Len:(1,0)
    ...
    , ISNULL(CAST(A.[KOSTL] COLLATE DATABASE_DEFAULT AS VARCHAR(10)),'')  AS [KOSTL]  -- X-Centro de coste-Check:CSKS-Datatype:CHAR-Len:(10,0)
    ...
    , ISNULL(CAST(A.[LTEXT] COLLATE DATABASE_DEFAULT AS VARCHAR(40)),'')  AS [LTEXT]  --  -Descripción-Check: -Datatype:CHAR-Len:(40,0)
...
FROM {{ var('P_SAPPRD_LS') }}.{{ source('SAPPRD', 'CSKT')}} A
WHERE A.MANDT = '300'
  AND A.SPRAS IN ('S')
