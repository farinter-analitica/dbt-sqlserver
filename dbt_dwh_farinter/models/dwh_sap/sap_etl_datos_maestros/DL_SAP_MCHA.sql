{% set unique_key_list = ["MATNR","WERKS","CHARG"] %}
{{ 
    config(
...
SELECT ISNULL(CAST(A.[MATNR] COLLATE DATABASE_DEFAULT AS VARCHAR(18)),'')  AS [MATNR]  -- X-Número de material-Check:MARA-Datatype:CHAR-Len:(18,0)
    ...
    , ISNULL(CAST(A.[CHARG] COLLATE DATABASE_DEFAULT AS VARCHAR(10)),'')  AS [CHARG]  -- X-Número de lote-Check:MCH1-Datatype:CHAR-Len:(10,0)
    , ISNULL(CAST(A.[LVORM] COLLATE DATABASE_DEFAULT AS VARCHAR(1)),'')  AS [LVORM]  --  -Petición de borrado para datos (todos) de lote de centro-Check: -Datatype:CHAR-Len:(1,0)
    ...
    , ISNULL(CAST(A.[VLCHA] COLLATE DATABASE_DEFAULT AS VARCHAR(10)),'')  AS [VLCHA]  --  -Número de lote original (desactivado)-Check:MCHA-Datatype:CHAR-Len:(10,0)
    ...
    , ISNULL(CAST(A.[BWTAR] COLLATE DATABASE_DEFAULT AS VARCHAR(10)),'')  AS [BWTAR]  --  -Clase de valoración-Check:MBEW-Datatype:CHAR-Len:(10,0)
    , ISNULL(CAST(A.[CHAME] COLLATE DATABASE_DEFAULT AS VARCHAR(3)),'')  AS [CHAME]  --  -Unidad de medida de salida del lote (desactivado)-Check:T006-Datatype:UNIT-Len:(3,0)
...
FROM {{ var('linked_server') }}.{{ source('SAPPRD', 'MCHA')}} A
