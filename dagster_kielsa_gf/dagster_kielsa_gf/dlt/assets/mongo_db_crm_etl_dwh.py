...
connection_str_source: str = EnvVar(
...
class DltSourceConfig:
    ...
    initial_value: pendulum.DateTime = Field(
        default_factory=lambda: get_for_current_env(
            {
...
read_source_config_updated_at: DltSourceConfigResourceList = {
    DltSourceConfig(
        ...
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_updated_at",
    ): [
        "crm_email",
    ...
    DltSourceConfig(
        ...
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_updatedat",
    ): [
        "crm_list",
    ],
}
...
read_source_config_multi_column: DltSourceConfigResourceList = {
    DltSourceConfig(
        cursor_path="updated_at",
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_updated_at",
    ...
    DltSourceConfig(
        ...
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_created_at",
        dep="mongo_crm_hn_multi_updated_at",
    ): [
        "crm_person",  ##pedir que actualicen de ser necesario para que funcione con updated_at en todos los docs
        "crm_message",  ##pedir que actualicen de ser necesario para que funcione con updated_at en todos los docs
        "crm_campaign",  ##pedir que actualicen de ser necesario para que funcione con updated_at en todos los docs
    ...
    DltSourceConfig(
        cursor_path="EndDate",
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_enddate",
        initial_value=pendulum.now().subtract(months=1),
    ): [
    ...
    DltSourceConfig(
        ...
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_createddate",
        dep="mongo_crm_hn_multi_enddate",
    ): [
        "campaignSchedule",  ##pedir que actualicen de ser necesario para que funcione con updated_at en todos los docs
    ],
    DltSourceConfig(
        cursor_path="updatedAt",
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_updatedat",
        initial_value=pendulum.now().subtract(months=1),
    ): [
    ...
    DltSourceConfig(
        ...
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_creationdate",
        dep="mongo_crm_hn_multi_updatedat",
    ): [
        "dataViewList",  ##pedir que actualicen de ser necesario para que funcione con updated_at en todos los docs
    ],
    DltSourceConfig(
    ...
    DltSourceConfig(
        ...
        primary_key="_id",
        pipeline_name_prefix="mongo_crm_hn_multi_createdat",
        dep="mongo_crm_hn_multi_updatedat",
    ): [
        "crmCall",  ##pedir que actualicen de ser necesario para que funcione con updated_at en todos los docs
        "campaignsRecetas",
...
read_source_config_not_incremental: DltSourceConfigResourceList = {
    DltSourceConfig(
        primary_key="_id", pipeline_name_prefix="mongo_crm_hn_not_incremental"
    ): [
        "campaignActivity",
    ],
}
...
table_remove_columns: dict[str, list[str]] = {
    "campaignsRecetas": ["created_at"],
}
...
all_mongo_db_source_configs: list[DltSourceConfigResourceList] = [
...
def get_config_filtered(
    dlt_source_config_resource_list: DltSourceConfigResourceList,
    dlt_source_config: DltSourceConfig,
) -> list[str]:
...
def dlt_mongo_db_crm_hn_asset_factory(
    mongo_db_source_configs: list[DltSourceConfigResourceList],
            ...
            for collection in collections:
                ...
                if collection in table_renames:
                    resource = resource.apply_hints(
                        table_name=table_renames[collection]
                    )

                if collection in table_columns_hints:
                    resource = resource.apply_hints(
                        columns=table_columns_hints[collection]
                    )

                if collection in table_remove_columns:
                    resource = resource.add_map(
                        lambda doc, columns=table_remove_columns[collection]: DagsterDltTranslatorMongodbCRMHN.remove_columns(
                ...
                if isinstance(config.primary_key, str):
                    resource = resource.apply_hints(
                        columns={
                            config.primary_key: {"data_type": "text", "precision": 50}
                        }
                    )
