...
comprobar_sinc_replicas_job_schedule = ScheduleDefinition(
    ...
    execution_timezone=default_timezone,
    job=job_control_replicas.comprobar_sinc_replicas_job,
...
def should_exec_kielsa_hourly_job_run(
    ...
    job_name: str = jobs.kielsa_hourly_job.name,
) -> bool:
    filters = RunsFilter(
        job_name=job_name,
...
kielsa_start_of_month_job_schedule = ScheduleDefinition(
    ...
    execution_timezone=default_timezone,
    job=jobs.kielsa_start_of_month_job,
    default_status=running_default_schedule_status,
)
...
def should_exec_kielsa_hourly_additional_job_run(
    context: ScheduleEvaluationContext,
    job_name: str = jobs.kielsa_hourly_additional_job.name,
) -> bool:
    filters = RunsFilter(
        job_name=job_name,
        statuses=[DagsterRunStatus.STARTED],
...
if __name__ == "__main__":
    ...
    context = build_schedule_context(
        scheduled_execution_time=datetime.now() - timedelta(days=1)
    )
